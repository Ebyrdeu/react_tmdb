...
const MovieCardsTypes = ({data, cardType}) => {
	const item1 = data.map(({id, title, backdrop_path, poster_path, vote_average, vote_count, release_date}) => {
	...
	const item2 = data.map(({id, title, poster_path, vote_average, release_date, overview}) => {
	...
	const item3 = data.map(({id, title, poster_path, vote_average, vote_count, release_date}, i) => {
		const renderedCards =
			<SlideInLeftDiv children={<Card
				as={Link}
				to={`info/${id}`}
	...
	return (
		<>
			...
			<Box sx={{display: 'flex', justifyContent: 'center', flexDirection: 'column', alignItems: 'center'}}>
				<Typography variant='subtitle2' sx={{color: '#647380',  mt: 5,  userSelect: 'none'}} children={'Scroll down to load more movies'}/>
